...
try:
    ...
    # Summary only in console (full text to file)
...
try:
    out(f"  NRFI board:     {len(nrfi_results)} games")
except: pass
try:
    out(f"  Hits board:     {len(hits_results)} pitcher slots")
except: pass
try:
    out(f"  TB board:       {len(tb_results)} batter rows")
except: pass
try:
    out(f"  F5 board:       {len(f5_results)} games")
except: pass
try:
    out(f"  Batter K board: {len(bk_results)} batter rows")
except: pass
out(f"\n  Files saved to: {SP}")
